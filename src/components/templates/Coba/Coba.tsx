...
// import { availableFontsCoBa,availableValuesCoBa} from '../../../config'
import { availableFontsCoBa, availableValuesCoBa as defaultValuesCoBa } from '../../../config';
...
const Coba: React.FC<CobaProps> = ({ disabled = false, userIdsub, themeIdsub }) => {
    ...
    // const [availableValuesCoBa, setAvailableValuesCoBa] = useState<Section[]>([]);
    ...
    // const [sections, setSections] = useState<Section[]>(availableValuesCoBa);
...
//         // Chỉ set sections nếu chưa có giá trị
...
//   if (availableValuesCoBa.length === 0) {
//     fetchDefaultData();
//   }
// }, []);
  ...
  useEffect(() => {
    ...
    //     }, []); 
      //subdomain
        ...
        const resolvedUserId =userIdsub ?? (params.userId ? parseInt(params.userId) : undefined);
        const resolvedThemeId = themeIdsub ?? (params.themeId ? parseInt(params.themeId) : undefined);
//    useEffect(() => {
//     const fetchData = async () => {
//       try {
...
//               availableFontsCoBa.find((f: FontOption) => f.name === dataFontTitle) ||
...
//           if (dataFontContent) {
//             const font: FontOption =
//               availableFontsCoBa.find((f: FontOption) => f.name === dataFontContent) ||
//               availableFontsCoBa[0];
//             setContentFont(font);
//           }
...
//         // Bước 3: Nếu không có dữ liệu từ /api/widgets, gọi /api/admin/documents/1
//         if (!hasWidgetData) {
//           await fetchDefaultData(); // *** SỬA: Chỉ gọi fetchDefaultData nếu không có dữ liệu từ /api/widgets ***
//         }

//         // *** THÊM: Set isDisabled dựa trên prop disabled ***
//         setIsDisabled(disabled);
//       } catch (error: unknown) {
//         if (error instanceof Error) {
//           console.error('Lỗi khi lấy dữ liệu:', error.message);
//           toast.error('Không thể lấy dữ liệu từ API widgets.');
//           await fetchDefaultData(); // *** SỬA: Gọi fetchDefaultData khi /api/widgets thất bại ***
//         }
//         // *** THÊM: Set isDisabled ngay cả khi có lỗi ***
//         setIsDisabled(disabled);
//       }
//     };

//     fetchData();
//   }, []); // *** SỬA: Thêm disabled vào dependencies ***
    ...
    const fetchData = async () => {
        try {
            ...
            let shouldDisable = false; // Theo dõi trạng thái vô hiệu hóa

            // Bước 1: Gọi /api/widgets
            if (themeId && userId) {
                response = await axios.get(`${API_BASE_URL}/api/widgets/${userId}/${themeId}`);
               
                shouldDisable = true; // Vô hiệu hóa khi share
            } else if (resolvedUserId && resolvedThemeId) {
                response = await axios.get(`${API_BASE_URL}/api/widgets/${resolvedUserId}/${resolvedThemeId}`);
                ...
                shouldDisable = false; // Cho phép chỉnh sửa
            } else {
                console.log("Không có điều kiện nào khớp, không gọi API");
            }
            ...
            // Bước 4: Set isDisabled dựa trên logic share
            setIsDisabled(shouldDisable);
        } catch (error: unknown) {
            console.error('Lỗi khi lấy dữ liệu:', error);
            toast.error('Không thể lấy dữ liệu từ API widgets.');
            await fetchDefaultData();
            // Mặc định vô hiệu hóa khi lỗi để an toàn
            setIsDisabled(true);
        }
    };
